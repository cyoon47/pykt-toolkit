...
class QDKTNet(nn.Module):
    def __init__(self, num_q,num_c,emb_size, dropout=0.1, emb_type='qaid', emb_path="", pretrain_dim=768,device='cuda',mlp_layer_num=1,other_config={}):
        super().__init__()
    ...
    def forward(self, q, c ,r,data=None):
        ...
        y = self.out_layer(h)
        y = torch.sigmoid(y)
        y = (y * F.one_hot(data['qshft'].long().to(self.device), self.num_q)).sum(-1)
        outputs = {"y":y}
        return outputs

class QDKT(QueBaseModel):
    def __init__(self, num_q,num_c, emb_size, dropout=0.1, emb_type='qaid', emb_path="", pretrain_dim=768,device='cuda',seed=0,mlp_layer_num=1,other_config={},**kwargs):
        model_name = "qdkt"
    ...
    def train_one_step(self,data,process=True,return_all=False):
        ...
        loss = self.get_loss(outputs['y'],data_new['rshft'].to(self.device),data_new['sm'].to(self.device))
